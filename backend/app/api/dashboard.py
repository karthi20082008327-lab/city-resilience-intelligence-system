...
async def dashboard_overview(db: AsyncSession = Depends(get_db)):
    ...
    critical = (await db.execute(
        select(func.count(Incident.id)).where(Incident.priority == "critical", Incident.status != "resolved", Incident.status != "closed")
    )).scalar() or 0
    ...
    for dept_name in departments:
        dept_total = (await db.execute(
            select(func.count(Incident.id)).where(Incident.assigned_department == dept_name)
        )).scalar() or 0
        dept_active = (await db.execute(
            select(func.count(Incident.id)).where(
    ...
    return {
        ...
        "ai_insights": [
            {"type": "warning", "message": f"{critical} critical incidents require immediate attention", "severity": "high"},
            {"type": "info", "message": f"{active_incidents} incidents are currently active across all departments", "severity": "medium"},
