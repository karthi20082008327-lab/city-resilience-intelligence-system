...
export default function MobileDetectPage() {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    ws.onopen = () => {
      ...
      ws.send(JSON.stringify({ type: 'config', camera_name: cameraName, latitude: 11.3128, longitude: 77.4909 }))
    }
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'status') {
          ...
          if (msg.alert) { setAlert(msg.alert); setTimeout(() => setAlert(null), 15000) }
        }
      } catch {}
    }
    ws.onclose = () => { setWsStatus('disconnected'); setTimeout(connectWebSocket, 3000) }
    ws.onerror = () => ws.close()
    wsRef.current = ws
  }, [cameraName, getWsUrl])
  ...
  const sendFrame = useCallback(() => {
    ...
    if (ctx) {
      ...
      canvas.toBlob((blob) => {
        ...
        } else { detectingRef.current = false }
      }, 'image/jpeg', 0.4)
    } else { detectingRef.current = false }
  }, [])
  ...
  const classColors: Record<string, string> = {
    person: '#22c55e', car: '#3b82f6', motorcycle: '#f59e0b', bus: '#8b5cf6', truck: '#ef4444', vehicle: '#3b82f6',
  }
      ...
      <header className="bg-[rgba(10,15,26,0.9)] backdrop-blur-xl border-b border-white/[0.06] px-4 py-3 flex items-center justify-between sticky top-0 z-20">
        ...
        <div className="flex items-center gap-3 text-xs">
          ...
          <span className={`px-2 py-0.5 rounded-full font-medium ${
            ...
            {wsStatus === 'connected' ? 'AI CONNECTED' : wsStatus === 'connecting' ? 'CONNECTING...' : 'OFFLINE'}
          </span>
        </div>
      </header>
      ...
      <main className="relative">
        <div className="relative w-full" style={{ aspectRatio: '4/3' }}>
          ...
          {!cameraActive && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/80">
              <div className="text-center px-6">
                ...
                <button onClick={startCamera} className="uc-btn uc-btn-primary px-6 py-2.5">Start Monitoring</button>
              </div>
            </div>
          )}
          ...
          {cameraActive && (
            <>
              ...
              {alert && (
                <div className={`absolute bottom-3 left-3 right-3 backdrop-blur-sm rounded-xl p-4 border ${
                  alert.type === 'accident' ? 'bg-red-950/90 border-red-500/30' : 'bg-orange-950/90 border-orange-500/30'
                }`}>
                  <p className={`font-bold text-sm ${alert.type === 'accident' ? 'text-red-200' : 'text-orange-200'}`}>
                    {alert.type === 'accident' ? 'ACCIDENT DETECTED' : alert.type === 'fire' ? 'FIRE DETECTED' : 'SMOKE DETECTED'}
                  </p>
                  <p className={`text-xs ${alert.type === 'accident' ? 'text-red-300/70' : 'text-orange-300/70'}`}>
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="uc-card p-3 text-center">
              <p className={`text-xl font-bold ${wsStatus === 'connected' ? 'text-purple-400' : 'text-white/20'}`}>
                ...
                {trackedObjects.map((obj) => (
                  <div key={obj.id} className="flex justify-between text-sm items-center py-1">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: classColors[obj.class] || '#fff' }} />
                      <span className="text-white/50 capitalize text-xs">{obj.class}</span>
                      <span className="text-white/20 text-[10px]">#{obj.id}</span>
                    </div>
                    <span className="text-emerald-400 font-mono text-xs">{(obj.confidence * 100).toFixed(0)}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-3">
            {cameraActive ? (
              <button onClick={stopCamera} className="flex-1 py-3 rounded-xl text-sm font-medium bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-colors">
                Stop Monitoring
              </button>
            ) : (
              <button onClick={startCamera} className="flex-1 py-3 rounded-xl text-sm font-medium uc-btn-primary">
                Start Monitoring
              </button>
            )}
          </div>
