...
export default function CCTVPage() {
  ...
  useEffect(() => {
    const fetchCollisions = async () => {
      try { const res = await incidentAPI.list({ category: 'accident', per_page: 5 }); setCollisionIncidents(res.data.incidents || []) } catch {}
    }
    fetchCollisions()
  }, [])
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/35 text-sm mt-0.5">{simulatorCameras.length} simulation cameras live · City Digital Twin</p>
        </div>
        <a href="/simul/" target="_blank" rel="noopener noreferrer" className="uc-btn uc-btn-primary text-xs py-2 px-3">
          Open Digital Twin <ExternalLink className="w-3.5 h-3.5" />
        </a>
      </div>

      {/* Mobile Detection */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="uc-card p-6 border border-blue-500/10">
        <div className="flex items-center justify-between mb-5">
          ...
          <a href="/detect" target="_blank" rel="noopener noreferrer"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            { label: 'How to use', text: 'Open the detector on your phone, point at traffic, and collisions are auto-reported.' },
            { label: 'AI Model', text: 'YOLOv8 + TensorFlow.js runs entirely in your browser. No video is streamed.' },
            { label: 'Auto-Report', text: 'When a collision is detected, a screenshot is captured and an incident is created.' },
          ].map((item) => (
          ...
          <div className="mt-4">
            ...
            <div className="space-y-1.5">
              {collisionIncidents.map((inc: any) => (
                <div key={inc.id} className="flex items-center gap-3 p-3 rounded-xl bg-red-500/[0.04] border border-red-500/10">
                  ...
                  {inc.snapshot_url && (
                    <img src={inc.snapshot_url} alt="" className="w-9 h-9 rounded-lg object-cover ring-1 ring-white/10" />
                  )}
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xs font-semibold text-white/40 uppercase tracking-wider">City Digital Twin — Live Cameras</h2>
