  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="uc-card p-6">
        <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-5">24-Hour Risk Forecast</h3>
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={predictionData}>
            ...
            <Tooltip contentStyle={{ background: 'rgba(10,15,26,0.95)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', fontSize: '12px', color: '#fff' }} />
            <Area type="monotone" dataKey="flood" stroke="#3b82f6" fill="url(#flood)" strokeWidth={2} name="Flood" />
            <Area type="monotone" dataKey="fire" stroke="#ef4444" fill="url(#fire)" strokeWidth={2} name="Fire" />
            <Area type="monotone" dataKey="accident" stroke="#eab308" fill="url(#accident)" strokeWidth={2} name="Accident" />
          </AreaChart>
        </ResponsiveContainer>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Risk Factors */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="uc-card p-6">
          <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-5">Active Risk Factors</h3>
          <div className="space-y-3">
            {riskFactors.map((factor, i) => (
              <div key={i} className="p-3.5 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] transition-colors border border-white/[0.03]">
                <div className="flex items-center justify-between mb-2.5">
                  <span className="text-sm text-white/80">{factor.name}</span>
                  <span className={`uc-chip ${
                    factor.impact === 'Critical' ? 'bg-red-500/10 text-red-400' :
                    factor.impact === 'High' ? 'bg-amber-500/10 text-amber-400' :
                    factor.impact === 'Medium' ? 'bg-blue-500/10 text-blue-400' :
                    'bg-emerald-500/10 text-emerald-400'
                  }`}>{factor.impact}</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-1.5 bg-white/[0.04] rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${factor.probability}%` }}
                      transition={{ duration: 1, delay: i * 0.08 }} className="h-full rounded-full" style={{ backgroundColor: factor.color }} />
                  </div>
                  <span className="text-xs text-white/40 w-10 text-right font-mono">{factor.probability}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Cascade Paths */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="uc-card p-6">
          <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-5">Cascade Propagation Paths</h3>
          <div className="space-y-2.5">
            {cascadingPaths.map((path, i) => (
              <div key={i} className="p-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-white/[0.08] transition-colors">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <span className={`text-xs font-semibold font-mono ${path.risk > 0.7 ? 'text-red-400' : path.risk > 0.5 ? 'text-amber-400' : 'text-emerald-400'}`}>
