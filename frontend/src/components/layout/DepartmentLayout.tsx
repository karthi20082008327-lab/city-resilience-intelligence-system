...
export default function DepartmentLayout() {
  ...
  const departmentName = getDepartmentName(user?.role?.name || '')
  const departmentColor = getDepartmentColor(user?.role?.name || '')
  ...
  const navItems = [
    ...
    { path: '/department/incidents', label: 'My Incidents', icon: AlertTriangle },
    { path: '/department/map', label: 'Live Map', icon: MapPin },
  ]
      ...
      >
        ...
        <AnimatePresence>
          {sidebarOpen && (
            ...
            >
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl ${departmentColor}`}>
        ...
        <nav className="flex-1 px-2.5 py-3 space-y-0.5 overflow-y-auto scrollbar-thin">
          {navItems.map((item) => {
            const isActive =
              ...
              (item.path === '/department/dashboard' && location.pathname === '/department')
            return (
              <NavLink
                key={item.path}
                to={item.path}
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative z-10">
        ...
        <header className="uc-glass h-14 flex items-center justify-between px-4 lg:px-6 sticky top-0 z-20 border-b border-slate-200/80">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl border ${departmentColor}`}>
              <Building2 className="w-3.5 h-3.5" />
              <span className="text-xs font-medium">{departmentName}</span>
            </div>
                ...
                {profileOpen && (
                  ...
                  >
                    <div className="px-3 py-2.5 border-b border-slate-200/80 mb-1">
                      ...
                      <p className="text-xs text-slate-500 truncate">{departmentName}</p>
                    </div>
                    <button
...
function getDepartmentColor(dept: string): string {
  const colors: Record<string, string> = {
    emergency_department: 'bg-red-500/10 text-red-700 border-red-500/20',
    traffic_department: 'bg-yellow-500/10 text-yellow-700 border-yellow-500/20',
    water_department: 'bg-blue-500/10 text-blue-700 border-blue-500/20',
    electricity_department: 'bg-purple-500/10 text-purple-700 border-purple-500/20',
    disaster_management: 'bg-teal-500/10 text-teal-700 border-teal-500/20',
  }
  return colors[dept] || 'bg-slate-500/10 text-slate-700 border-slate-500/20'
}
